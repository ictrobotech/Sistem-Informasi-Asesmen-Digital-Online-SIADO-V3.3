...
async function exportLaporanFile(format) {
  ...
  status.className = 'media-status show info';
  status.innerHTML = '<i class="fa-solid fa-circle-notch fa-spin"></i> Menyusun laporan ' + (format === 'pdf' ? 'PDF' : 'Excel') + '. Proses ini bisa memakan waktu beberapa detik...';
  try {
    var result = await adminApi('exportLaporan', {
      format: format,
      kelas: document.getElementById('exportKelas').value.trim()
    });
    if (!guardAdminResult(result)) return;
    if (!result.success) throw new Error(result.message || 'Export gagal.');
    unduhBlob_(base64KeBlob_(result.base64, result.mimeType), result.fileName);
    status.className = 'media-status show ok';
    status.innerHTML = '<i class="fa-solid fa-circle-check"></i> Laporan <b>' + escapeAdmin(result.fileName) + '</b> berhasil diunduh.';
    ...
    await hasilSukses_('Laporan Berhasil Diunduh',
      'Berkas laporan tersimpan di folder unduhan perangkat Anda.', [
        { label: 'Nama berkas', nilai: result.fileName || '-' },
        { label: 'Format', nilai: format === 'pdf' ? 'PDF' : 'Excel' },
        { label: 'Cakupan', nilai: kelasDipilih ? ('Kelas ' + kelasDipilih) : 'Semua kelas' },
        { label: 'Jumlah baris', nilai: String(result.jumlah !== undefined ? result.jumlah : '-') }
      ]);
  } catch (error) {
    status.className = 'media-status show err';
    ...
    await hasilGagal_('Laporan Gagal Dibuat', error.message || 'Laporan gagal dibuat.', [
      { label: 'Format diminta', nilai: format === 'pdf' ? 'PDF' : 'Excel' }
    ]);
