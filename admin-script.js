...
function susunRencanaImportKartu_(rows) {
  ...
  for (var i = 1; i < rows.length; i++) {
    ...
    var nilai = {
      ...
      level: normalisasiLevelKognitifTeks_(ambil(obj, 'level_kognitif') || ambil(obj, 'level'))
    };
...
/** Membangun berkas template Excel kartu soal (terisi data bank soal). */
async function buatTemplateExcelKartu_() {
  ...
  urut.forEach(function(soal) {
    sheet.addRow({
      ...
      nomor_bank: peta[String(soal.id_soal)] || '',
      tipe: String(soal.tipe || '').toUpperCase(),
      mapel: soal.mapel || '',
      kelas: tingkatDariNilai_(soal.tingkat),
      pertanyaan_ringkas: truncate(SRich.stripHtml(soal.pertanyaan), 150),
      nomor_kartu: soal.ks_nomor_soal || '',
      capaian_pembelajaran: soal.ks_capaian || '',
      tujuan_pembelajaran: soal.ks_tujuan || '',
      materi_elemen: soal.ks_materi || '',
      kompetensi: soal.ks_kompetensi || '',
      indikator_soal: soal.ks_indikator || '',
      level_kognitif: soal.ks_level_kognitif || ''
    });
  });
  ...
  [
    ...
    '',
    '1. Sheet "KARTU SOAL" SUDAH TERISI data bank soal Anda: kolom id_soal sampai pertanyaan_ringkas adalah identitas butir (JANGAN diubah/dihapus), kolom nomor_kartu sampai level_kognitif adalah isi kartu yang boleh dilengkapi/diperbaiki.',
    '2. Isi kolom: nomor_kartu (angka, kosong = otomatis urut mapel+kelas), capaian_pembelajaran, tujuan_pembelajaran, materi_elemen (wajib bila mengisi deskripsi), kompetensi, indikator_soal, level_kognitif.',
    '3. level_kognitif boleh ditulis: C1..C6 (mis. "C4 - Menganalisis") atau Taksonomi SOLO: Prastruktural / Unistruktural / Multistruktural / Relasional / Abstrak Diperluas.',
    '4. Jangan menambah/menghapus baris identitas, mengganti header, atau mengubah nama sheet "KARTU SOAL".',
    '5. Simpan tetap sebagai .xlsx, kembali ke panel → Kartu Soal → Pilih Berkas → Import Kartu.',
    '6. Baris yang id-nya tidak dikenal atau tanpa isi baru akan DILEWATI dan dilaporkan pada hasil import.',
    '',
    'Catatan: butir yang sebelumnya sudah berisi kartu akan DIPERBARUI sesuai isi berkas (kolom yang dikosongkan di berkas mempertahankan nilai lama).'
...
async function unduhTemplateKartuSoal() {
  ...
  try {
    ...
    await hasilSukses_('Template Kartu Soal Diunduh (Excel)',
      ...
      'beserta nilai kartu yang sudah ada. Lengkapi kolom kartu di Excel (lihat sheet PETUNJUK), simpan, lalu Import Kartu.',
      [{ label: 'Nama berkas', nilai: 'template-import-kartu-soal.xlsx' },
       { label: 'Butir tercakup', nilai: String(daftar.length) }]);
  } catch (error) {
    var rows = [KARTU_TEMPLATE_HEADERS_].concat(daftar.map(function(s) {
      return [s.id_soal, '', String(s.tipe || '').toUpperCase(), s.mapel || '', tingkatDariNilai_(s.tingkat),
        truncate(SRich.stripHtml(s.pertanyaan), 150), s.ks_nomor_soal || '', s.ks_capaian || '', s.ks_tujuan || '',
        s.ks_materi || '', s.ks_kompetensi || '', s.ks_indikator || '', s.ks_level_kognitif || ''];
    }));
