...
function bindAdminInterface() {
  ...
  // REVISI 2026-09-26: import kartu soal via template Excel.
  siapkanImportKartuUi_();
...
/* ==================================================================
 * REVISI 7 — TABEL BANK SOAL GAYA DOKUMEN (RAPI)
 * Pengganti renderQuestionTable() versi REVISI 5. Mengikuti sampel guru:
 * garis penuh, kolom pendek rata tengah sebagai teks polos (tanpa
 * lencana), PERTANYAAN & OPSI/KUNCI rata kiri-kanan dan rata atas, satu
 * opsi per baris ditutup "Kunci :", tombol aksi di tengah sel.
 * Perataan memakai kelas sel (bs-tengah / bs-teks / bs-aksi), bukan
 * urutan kolom — aturan lama #questionTable td:nth-child(4)/(6) ikut
 * bergeser saat kolom NOMOR disisipkan (REVISI 5) dan kini tidak dipakai.
 * Bergantung pada: escapeAdmin, truncate, tryJson, SRich.stripHtml,
 * tingkatDariNilai_, petaNomorSoal_, urutkanSoalMenurutNomor_,
 * DATA_MENTAH, setTableMessage, openEditQuestion, deleteQuestion.
 * ================================================================== */

/** Label tipe berupa teks polos untuk tabel Bank Soal (sesuai sampel). */
function labelTipeBankSoal_(tipe) {
  var kode = String(tipe || '').toUpperCase();
  return {
    PG: 'PG', PGK: 'PGK Kategori', PGK_MCMA: 'PGK MCMA',
    MENJODOHKAN: 'Menjodohkan', ISIAN: 'Isian', URAIAN: 'Uraian'
  }[kode] || String(tipe || '-');
}

/** Kunci berbentuk objek/array (teks JSON atau sudah objek); null bila bukan. */
function kunciTeruraiBankSoal_(nilai) {
  if (nilai && typeof nilai === 'object') return nilai;
  var hasil = tryJson(nilai, null);
  return (hasil && typeof hasil === 'object') ? hasil : null;
}

/**
 * Satu baris opsi dengan penanda menggantung (huruf A./B. atau nomor 1./2.).
...
/**
 * Isi sel OPSI/KUNCI: satu opsi per baris, lalu baris "Kunci :".
 * PG/PGK MCMA: A. B. C. ...; PGK Kategori: pernyataan bernomor + kunci per
 * nomor; Menjodohkan: "pernyataan = pasangan"; Isian/Uraian: kunci saja.
 */
function selOpsiKunciBankSoal_(question) {
  var tipe = String(question.tipe || '').toUpperCase();
  var opsi = Array.isArray(question.opsi) ? question.opsi : [];
  var teks = function(nilai) { return truncate(SRich.stripHtml(nilai === undefined || nilai === null ? '' : nilai), 140); };
  var baris = '';
  var kunci = '';
  if (tipe === 'PG' || tipe === 'PGK_MCMA') {
    ...
    kunci = (Array.isArray(pilihan) ? pilihan : String(question.kunci_jawaban || '').split(','))
      .map(function(k) { return String(k).trim(); }).filter(Boolean).join(', ');
  } else if (tipe === 'PGK') {
    ...
    kunci = opsi.map(function(op, i) {
      ...
      if (daftar) k = daftar[i];
        ...
        k = peta[op.id !== undefined ? op.id : String(i)];
        if (k === undefined) k = peta[String(i)];
      }
      // Spasi tak-terputus: nomor pernyataan tidak terpisah dari kuncinya saat pindah baris.
      return (i + 1) + '.\u00a0' + (String(k === undefined || k === null ? '' : k).trim() || '-');
    }).join('; ');
    ...
    // ISIAN / URAIAN: tanpa opsi, cukup kunci/rubrik.
    kunci = SRich.stripHtml(String(question.kunci_jawaban === undefined || question.kunci_jawaban === null ? '' : question.kunci_jawaban));
  }
...
function selStimulusBankSoal_(question) {
  var isi = [];
  if (question.stimulus_gambar) isi.push('<span class="bs-media"><i class="fa-regular fa-image"></i> Gambar</span>');
  if (question.stimulus_video) isi.push('<span class="bs-media"><i class="fa-solid fa-film"></i> Video</span>');
  return isi.length ? isi.join('') : '<span class="bs-kosong">-</span>';
}
...
function renderQuestionTable(rows) {
  ...
  var html = '<table class="admin-table bank-soal-rapi"><colgroup>' +
    '<col class="bs-c-id"><col class="bs-c-nomor"><col class="bs-c-tipe"><col class="bs-c-mapel"><col class="bs-c-tanya">' +
    '<col class="bs-c-kelas"><col class="bs-c-opsi"><col class="bs-c-stimulus"><col class="bs-c-poin"><col class="bs-c-status"><col class="bs-c-aksi">' +
    '</colgroup><thead><tr><th>ID</th><th>Nomor</th><th>Tipe</th><th>Mapel</th><th>Pertanyaan</th><th>Kelas</th>' +
    '<th>Opsi/Kunci</th><th>Stimulus</th><th>Poin</th><th>Status</th><th>Aksi</th></tr></thead><tbody>';
  rows.forEach(function(question) {
    var nomorSoal = petaNomor[String(question.id_soal)];
    var id = escapeAdmin(question.id_soal);
    ...
    html += '<tr>' +
      ...
      '<td class="bs-tengah">' + escapeAdmin(tingkatDariNilai_(question.tingkat)) + '</td>' +
      '<td class="bs-teks">' + selOpsiKunciBankSoal_(question) + '</td>' +
      '<td class="bs-tengah">' + selStimulusBankSoal_(question) + '</td>' +
      '<td class="bs-tengah">' + escapeAdmin(question.poin) + '</td>' +
      '<td class="bs-tengah"><span class="bs-status ' + (aktif ? 'aktif' : 'nonaktif') + '">' + (aktif ? 'AKTIF' : 'NONAKTIF') + '</span></td>' +
      '<td class="bs-tengah bs-aksi"><div class="row-actions">' +
        '<button class="mini-button edit" data-edit-question="' + id + '" type="button"><i class="fa-solid fa-pen"></i> Edit</button>' +
        '<button class="mini-button delete" data-delete-question="' + id + '" type="button" aria-label="Hapus soal #' + id + '"><i class="fa-solid fa-trash"></i></button>' +
      '</div></td></tr>';
  });
  html += '</tbody></table>';
  var wadah = document.getElementById('questionTable');
  wadah.classList.add('bs-rapi');   // bingkai kontainer mengikuti garis tabel
...
function perbaruiInfoMapelSoal_() {
  ...
  el.innerHTML = '<i class="fa-solid fa-circle-info" style="color:#1f6feb"></i> ' +
    'Soal baru &amp; import tersimpan otomatis ke bank soal akun ini. Mapel aktif sekarang: <b>' +
    escapeAdmin(aktif || 'belum dipilih') + '</b><br>' +
    '<span style="font-size:11px">Bank soal milik akun ini &mdash; ' + ringkas +
    '. Soal import masuk ke mapel yang dipilih pada kartu Import Soal (default: mapel aktif ini).</span>';
}
...
/* ==================================================================
 * REVISI 2026-09-24 (REVISI 4) — TEMPLATE IMPORT SOAL FORMAT EXCEL
 *
 * Template unduhan kini berupa berkas Excel asli (.xlsx) dengan dua
 * sheet: "SOAL" (header + 7 contoh semua tipe) dan "PETUNJUK"
 * (panduan isi per kolom), dibangun ExcelJS langsung di browser.
 * Struktur kolom mengikuti form edit soal realtime dan memuat kolom
 * `nomor` di antara `id` dan `tipe` sehingga guru/admin tidak perlu
...
/**
 * Membangun berkas template Excel (.xlsx) di browser memakai ExcelJS.
 * Mengembalikan Blob, atau null bila pustaka ExcelJS belum termuat
 * (panggilannya lalu jatuh ke template CSV cadangan).
 */
async function buatTemplateExcelSoal_() {
  var ExcelJS = window.ExcelJS;
  if (!ExcelJS || !ExcelJS.Workbook) return null;
  var workbook = new ExcelJS.Workbook();
  workbook.creator = 'SIADO — Sistem Informasi Asesmen Digitalisasi Online';
  workbook.lastModifiedBy = 'SIADO';
  workbook.properties.title = 'Template Import Soal SIADO';
  workbook.properties.description = 'Format panduan import soal: sheet SOAL (data) + PETUNJUK (panduan isi).';

  var sheet = workbook.addWorksheet('SOAL', { views: [{ state: 'frozen', ySplit: 1 }] });
  sheet.columns = SOAL_TEMPLATE_HEADERS_.map(function(h, i) {
    return { header: h, key: h, width: SOAL_TEMPLATE_LEBAR_[i] };
  });
  ...
  SOAL_TEMPLATE_CONTOH_.forEach(function(baris, i) {
    var row = sheet.getRow(2 + i);
    baris.forEach(function(nilai, j) {
      row.getCell(j + 1).value = (nilai === '' ? null : nilai);
    });
    row.eachCell({ includeEmpty: false }, function(cell) {
      var wrap = ['pertanyaan', 'opsi', 'kunci_jawaban'].indexOf(SOAL_TEMPLATE_HEADERS_[cell.col - 1]) !== -1;
      cell.alignment = { vertical: 'top', wrapText: wrap };
    });
  });
  sheet.dataValidations.add('B2:B200', {
    type: 'whole', operator: 'greaterThanOrEqual', formula1: '1', formula2: '99999',
    allowBlank: true, showErrorMessage: true, errorStyle: 'warning',
    errorTitle: 'Nomor soal', error: 'Nomor soal sebaiknya berupa angka positif.'
  });
  sheet.dataValidations.add('C2:C200', {
    type: 'list', allowBlank: true, showErrorMessage: true,
    formula1: '"PG,PGK,PGK_MCMA,MENJODOHKAN,ISIAN,URAIAN"',
    errorTitle: 'Tipe soal', error: 'Pilih salah satu tipe: PG, PGK, PGK_MCMA, MENJODOHKAN, ISIAN, URAIAN.'
  });
  sheet.dataValidations.add('H2:H200', {
    type: 'list', allowBlank: true, showErrorMessage: true,
    formula1: '"SEMUA,VII,VIII,IX"',
    errorTitle: 'Tingkat', error: 'Pilih SEMUA, VII, VIII, atau IX.'
  });
  sheet.dataValidations.add('I2:I200', {
    type: 'list', allowBlank: true, showErrorMessage: true,
    formula1: '"YA,TIDAK"',
    errorTitle: 'Status', error: 'Pilih YA atau TIDAK.'
  });

  var petunjuk = workbook.addWorksheet('PETUNJUK', { views: [{ state: 'frozen', ySplit: 1 }] });
  petunjuk.columns = [{ width: 6 }, { width: 110 }];
  petunjuk.mergeCells('A1:B1');
  var judul = petunjuk.getRow(1).getCell(1);
  judul.value = 'PETUNJUK ISI TEMPLATE IMPORT SOAL SIADO';
  judul.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF1F4E78' } };
  judul.font = { bold: true, color: { argb: 'FFFFFFFF' }, size: 12 };
  judul.alignment = { vertical: 'middle' };
  SOAL_TEMPLATE_PETUNJUK_.forEach(function(teks, i) {
    var row = petunjuk.getRow(2 + i);
    row.getCell(1).value = i + 1;
    row.getCell(2).value = teks;
...
async function unduhTemplateSoal() {
  var rincian = [
    { label: 'Pemisah opsi/pernyataan', nilai: 'tanda | (pipa)' },
  ...
  try {
    var blob = await buatTemplateExcelSoal_();
    if (!blob) throw new Error('Pustaka ExcelJS belum termuat.');
    unduhBlob_(blob, 'template-import-soal.xlsx');
    await hasilSukses_('Template Soal Diunduh (Excel)',
      'Berkas <b>template-import-soal.xlsx</b> tersimpan di folder unduhan perangkat Anda. Isi sesuai panduan pada sheet <b>PETUNJUK</b>, simpan, lalu tekan <b>Import Sekarang</b> pada kartu Import Soal.',
      [{ label: 'Nama berkas', nilai: 'template-import-soal.xlsx' }].concat(rincian));
  } catch (error) {
    var rows = [SOAL_TEMPLATE_HEADERS_].concat(SOAL_TEMPLATE_CONTOH_.map(function(b) {
      return b.map(function(nilai) { return String(nilai === null ? '' : nilai); });
    }));
    unduhCsv_(rows, 'template-import-soal.csv');
    await hasilInfo_('Template Diunduh (CSV cadangan)',
      'Pustaka Excel belum termuat pada perangkat ini, sehingga template diunduh sebagai CSV. ' +
      (error && error.message ? error.message : ''),
      [{ label: 'Nama berkas', nilai: 'template-import-soal.csv' }].concat(rincian));
  }
}
...
/** Membaca berkas biner (ArrayBuffer) — dipakai untuk file Excel. */
function bacaFileBinar_(file) {
  return new Promise(function(resolve, reject) {
    var reader = new FileReader();
    reader.onload = function() { resolve(reader.result); };
    reader.onerror = function() { reject(new Error('Berkas tidak dapat dibaca.')); };
...
/**
 * Mengubah berkas hasil pilih (Excel .xlsx/.xls atau CSV) menjadi daftar
 * baris. Sheet pertama dipakai; header dikenali dari baris pertama.
 */
async function barisDariBerkasSoal_(file) {
  var nama = String(file.name || '');
  if (/\.(xlsx|xls)$/i.test(nama)) {
    if (!window.XLSX || !window.XLSX.read || !window.XLSX.utils) {
      throw new Error('Pustaka pembaca Excel belum termuat. Muat ulang halaman panel, lalu coba lagi.');
    }
    var buffer = await bacaFileBinar_(file);
    var buku = window.XLSX.read(buffer, { type: 'array' });
    if (!buku.SheetNames || !buku.SheetNames.length || !buku.Sheets[buku.SheetNames[0]]) {
      throw new Error('Berkas Excel tidak berisi sheet apa pun.');
    }
    return window.XLSX.utils.sheet_to_json(buku.Sheets[buku.SheetNames[0]], { header: 1, defval: '' });
  }
  return parseCsv_(await bacaFileTeks_(file));
}

/**
 * Menyusun payload import dari baris objek. REVISI 4:
 * - `mapel` setiap baris DIPAKSA ke mapel tujuan (mapel yang diampu akun
 *   ini) — kolom mapel pada berkas diabaikan;
 * - baris diurutkan berdasarkan kolom `nomor` (baris tanpa nomor
 *   mengikuti urutan berkas di akhir), sehingga susunan bank soal sesuai
 *   penomoran guru;
 * - awalan nomor lama ("1. ...") dihilangkan dari teks pertanyaan agar
 *   kolom pertanyaan tetap bersih.
 */
function susunPayloadImportSoal_(objects, mapelTujuan) {
  var mapel = String(mapelTujuan || '').trim();
  var diberiNomor = (objects || []).map(function(item, index) {
    var nomorTeks = String(item.nomor || item.nomor_soal || item.no || '').trim();
    var bersih = nomorTeks.replace(/[^0-9]/g, '');
    return {
      index: index,
      nomor: (nomorTeks !== '' && bersih !== '' && !isNaN(Number(bersih))) ? Number(bersih) : null,
      item: item
    };
  });
  diberiNomor.sort(function(a, b) {
    if (a.nomor === null && b.nomor === null) return a.index - b.index;
    if (a.nomor === null) return 1;
    if (b.nomor === null) return -1;
    return (a.nomor - b.nomor) || (a.index - b.index);
  });
  var nomorMin = null, nomorMaks = null;
  var payload = diberiNomor.map(function(entri) {
    var item = entri.item;
    var pertanyaan = String(item.pertanyaan || item.soal || '').trim();
    if (pertanyaan) {
      var pola;
      if (entri.nomor !== null) {
        var angka = String(entri.nomor).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        pola = new RegExp('^' + angka + '[.)\\-](?=\\s|$)', 'i');
      } else {
        pola = /^\d{1,4}[.)\\-](?=\s|$)/;
      }
      if (pola.test(pertanyaan)) pertanyaan = pertanyaan.replace(pola, '').trim();
    }
    if (entri.nomor !== null) {
      if (nomorMin === null || entri.nomor < nomorMin) nomorMin = entri.nomor;
      if (nomorMaks === null || entri.nomor > nomorMaks) nomorMaks = entri.nomor;
    }
    return {
      tipe: String(item.tipe || item.jenis || 'PG').toUpperCase().replace(/[\s-]+/g, '_'),
      pertanyaan: pertanyaan,
      opsi: String(item.opsi || item.pilihan || '').trim(),
      kunci_jawaban: String(item.kunci_jawaban || item.kunci || '').trim(),
      poin: item.poin || item.skor || 10,
      // Mapel dari pengaturan akun, bukan dari berkas (lihat REVISI 4).
      mapel: mapel,
      stimulus_gambar: String(item.stimulus_gambar || item.gambar || '').trim(),
      stimulus_video: String(item.stimulus_video || item.video || '').trim(),
      stimulus_deskripsi: String(item.stimulus_deskripsi || item.deskripsi_stimulus || '').trim(),
  ...
  return { payload: payload, nomorMin: nomorMin, nomorMaks: nomorMaks };
}

/** Pembaca nilai dropdown "Mapel tujuan" pada kartu Import Soal. */
function nilaiMapelImport_() {
  var el = document.getElementById('importSoalMapel');
  if (el) return String(el.value || '').trim();
  return mapelDiujikan_();
}

/**
 * Mengisi dropdown "Mapel tujuan" dengan daftar mapel yang diampu akun ini
 * (dari pengaturan admin, maks. 2 untuk guru). Pilihan lama dipertahankan
 * bila masih ada; default = mapel aktif bila termasuk daftar.
 */
function isiPilihanMapelImport_() {
  var el = document.getElementById('importSoalMapel');
  if (!el) return;
  var sebelum = String(el.value || '').trim();
  var daftar = daftarMapelMilik_(mapelAktifTerpilih_() || (ADMIN.settings && ADMIN.settings.mapel) || '');
  var html = daftar.map(function(m) {
    return '<option value="' + escapeAdmin(m) + '">' + escapeAdmin(m) + '</option>';
  }).join('');
  if (!daftar.length) html = '<option value="">Belum ada mapel — tetapkan di Pengaturan</option>';
  el.innerHTML = html;
  var cocok = daftar.filter(function(m) { return m.toLowerCase() === sebelum.toLowerCase(); })[0];
...
async function importSoalDariFile() {
  ...
  if (!file) {
    setImportMessage_('importSoalResult', 'Pilih berkas terlebih dahulu.', 'err');
    await hasilInfo_('Berkas Belum Dipilih', 'Pilih berkas Excel (.xlsx) hasil unduh template — atau CSV lama — lalu tekan Import Sekarang.');
    return;
  }
  // Cadangan pengisian bila tab dibuka tanpa pemuatan bank soal (mis. sesi baru).
  if (!document.getElementById('importSoalMapel') || !String(document.getElementById('importSoalMapel').value || '').trim()) {
    isiPilihanMapelImport_();
  }
  var mapelTujuan = nilaiMapelImport_();
  if (!mapelTujuan) {
    setImportMessage_('importSoalResult', 'Pilih <b>mapel tujuan</b> pada dropdown di atas file terlebih dahulu.', 'err');
    await hasilInfo_('Mapel Tujuan Belum Dipilih',
      'Soal hasil import otomatis masuk ke bank soal akun ini sesuai mapel yang diampu. Bila dropdown masih kosong, tetapkan dulu mapel pada Pengaturan.', [
        { label: 'Berkas dipilih', nilai: file.name }
      ]);
    return;
  }
  if (/\.(docx?|odt|rtf|pages)$/i.test(file.name)) {
    setImportMessage_('importSoalResult', 'Berkas Word tidak dapat dibaca langsung. Simpan sebagai <b>Excel (.xlsx)</b> lalu import kembali.', 'err');
    await hasilInfo_('Format Word',
      'Buka berkas di Word/Google Docs, lalu pilih Simpan/Unduh Sebagai Excel (.xlsx) — atau salin tabel soalnya ke Excel mengikuti struktur template — dan import kembali.', [
        { label: 'Berkas dipilih', nilai: file.name }
      ]);
    return;
  }
  ...
  try {
    var rows = await barisDariBerkasSoal_(file);
    var objects = barisKeObjek_(rows);
    if (!objects.length) throw new Error('Berkas tidak berisi data soal.');
    var susunan = susunPayloadImportSoal_(objects, mapelTujuan);
    if (!susunan.payload.length) throw new Error('Tidak ada baris soal yang terbaca pada berkas.');
    var result = await apiWajib_('importSoalBatch', { rows: susunan.payload });
    ...
    var labelNomor = susunan.nomorMin === null
      ? 'urutan berkas'
      : (susunan.nomorMaks > susunan.nomorMin ? susunan.nomorMin + '–' + susunan.nomorMaks : String(susunan.nomorMin));
    await (jumlahGagal ? hasilInfo_ : hasilSukses_)(
      ...
      (jumlahGagal
        ...
        : 'Seluruh soal tersimpan otomatis ke bank soal akun ini. ') +
      'Stimulus gambar/video dapat diunggah kemudian lewat tombol <b>Edit</b> pada Bank Soal.',
      [ { label: 'Mapel tujuan', nilai: mapelTujuan },
        { label: 'Nomor soal', nilai: labelNomor },
        { label: 'Baris dibaca', nilai: String(susunan.payload.length) },
...
/* ==================================================================
 * IMPORT KARTU SOAL VIA TEMPLATE EXCEL (REVISI 2026-09-26)
 * Guru mengunduh template yang SUDAH TERISI data bank soal (id, tipe,
 * mapel, kelas, pertanyaan, dan nilai kartu yang ada), melengkapinya
 * di Excel, lalu mengimpor kembali — setiap baris dipetakan ke butir
 * soal lewat id_soal dan disimpan lewat action simpanKartuSoal yang
 * sama dengan modal, termasuk normalisasi kunci & level kognitif.
 * ================================================================== */
var KARTU_TEMPLATE_HEADERS_ = ['id_soal', 'nomor_bank', 'tipe', 'mapel', 'kelas',
  'pertanyaan_ringkas', 'nomor_kartu', 'capaian_pembelajaran', 'tujuan_pembelajaran',
  'materi_elemen', 'kompetensi', 'indikator_soal', 'level_kognitif'];

/** Keterangan pengganti isi kartu pada template: guru MENIMPA teks ini dengan isi sebenarnya.
 * Sengaja TIDAK memuat kode C1..C6 agar tidak salah terbaca sebagai level kognitif. */
var KARTU_PLACEHOLDER_ = {
  nomor_kartu: '(isi nomor kartu di sini — kosongkan bila ingin otomatis)',
  capaian_pembelajaran: '(isi capaian pembelajaran di sini)',
  tujuan_pembelajaran: '(isi tujuan pembelajaran di sini)',
  materi_elemen: '(isi materi/elemen di sini)',
  kompetensi: '(isi kompetensi di sini)',
  indikator_soal: '(isi indikator soal di sini)',
  level_kognitif: '(isi level kognitif di sini)'
};

/** Benar bila nilai sel masih teks keterangan template (atau kosong) — dianggap belum diisi guru. */
function adalahPlaceholderKartu_(nilai) {
  var s = String(nilai === undefined || nilai === null ? '' : nilai).replace(/\s+/g, ' ').trim().toLowerCase();
  if (!s) return true;
  if (s.charAt(0) !== '(') return false;
  if (/^\(isi /.test(s) || /^\(kosongkan /.test(s)) return true;
  return Object.keys(KARTU_PLACEHOLDER_).some(function(k) {
    return KARTU_PLACEHOLDER_[k].replace(/\s+/g, ' ').trim().toLowerCase() === s;
  });
}

/** Gabungan data kartu + bank soal untuk satu id (kartu lebih dulu). */
function sumberKartuGabungan_() {
  var peta = {};
  (ADMIN.questions || []).concat(DATA_MENTAH.soal || []).forEach(function(q) {
    if (q && q.id_soal !== undefined) peta[String(q.id_soal)] = Object.assign({}, peta[String(q.id_soal)] || {}, q);
  });
  (KARTU_SOAL.data || []).forEach(function(q) {
    if (q && q.id_soal !== undefined) peta[String(q.id_soal)] = Object.assign({}, peta[String(q.id_soal)] || {}, q);
  });
  return Object.keys(peta).map(function(k) { return peta[k]; });
}

/** Kunci kolom toleran: huruf kecil, spasi/garis jadi garis bawah. */
function kunciKolomKartu_(nama) {
  return String(nama || '').trim().toLowerCase().replace(/[\s\-./]+/g, '_').replace(/_+/g, '_');
}

/** Menormalkan teks level kognitif dari Excel menjadi nilai resmi. */
function normalisasiLevelKognitifTeks_(teks) {
  var s = String(teks === undefined || teks === null ? '' : teks).trim();
  if (!s) return '';
  var kode = /C[1-6]/i.exec(s);
  if (kode) return kode[0].toUpperCase();
  var low = s.toLowerCase();
  var solo = { 'prastruktural': 'Prastruktural', 'unistruktural': 'Unistruktural',
    'multistruktural': 'Multistruktural', 'relasional': 'Relasional',
    'abstrak diperluas': 'Abstrak Diperluas' };
  return solo[low] || '';
}

/** Membaca baris Excel/CSV menjadi array array (baris pertama = header). */
async function barisDariBerkasKartu_(file) {
  var nama = String(file.name || '');
  if (/\.csv$/i.test(nama)) return parseCsv_(await bacaFileTeks_(file));
  if (!window.XLSX || !window.XLSX.read || !window.XLSX.utils) {
    throw new Error('Pustaka pembaca Excel belum termuat. Muat ulang halaman panel, lalu coba lagi.');
  }
  var buffer = await bacaFileBinar_(file);
  var buku = window.XLSX.read(buffer, { type: 'array' });
  var namaSheet = buku.SheetNames.filter(function(n) { return /kartu/i.test(n); })[0] || buku.SheetNames[0];
  if (!namaSheet || !buku.Sheets[namaSheet]) throw new Error('Berkas Excel tidak berisi sheet apa pun.');
  return window.XLSX.utils.sheet_to_json(buku.Sheets[namaSheet], { header: 1, defval: '' });
}

/**
 * Menyusun rencana import: { entri: [...], dilewati: [...] }.
 * Setiap entri berisi { baris, id, muatan } — muatan siap dikirim ke
 * action simpanKartuSoal. Baris tanpa perubahan / id tak dikenal dilewati.
 */
function susunRencanaImportKartu_(rows) {
  var hasil = { entri: [], dilewati: [] };
  if (!rows || rows.length < 2) {
    hasil.dilewati.push('Berkas tidak berisi baris data.');
    return hasil;
  }
  var header = (rows[0] || []).map(kunciKolomKartu_);
  var indeksId = header.indexOf('id_soal');
  if (indeksId === -1) indeksId = header.indexOf('id');
  if (indeksId === -1) {
    hasil.dilewati.push('Kolom id_soal tidak ditemukan pada baris pertama berkas.');
    return hasil;
  }
  function ambil(obj, nama) { return String(obj[nama] === undefined || obj[nama] === null ? '' : obj[nama]).trim(); }

  var daftarSoal = sumberKartuGabungan_();
  var petaSoal = {};
  daftarSoal.forEach(function(q) { petaSoal[String(q.id_soal)] = q; });

  for (var i = 1; i < rows.length; i++) {
    var baris = rows[i] || [];
    if (!baris.length || baris.every(function(c) { return String(c).trim() === ''; })) continue;
    var obj = {};
    header.forEach(function(k, idx) { if (k) obj[k] = baris[idx]; });
    var id = ambil(obj, 'id_soal') || ambil(obj, 'id');
    if (!id) { hasil.dilewati.push('Baris ' + (i + 1) + ': id_soal kosong — dilewati.'); continue; }
    var soal = petaSoal[id];
    if (!soal) { hasil.dilewati.push('Baris ' + (i + 1) + ': id #' + id + ' tidak ada pada bank soal — dilewati.'); continue; }

    var nilai = {
      nomor: ambil(obj, 'nomor_kartu') || ambil(obj, 'nomor'),
      capaian: ambil(obj, 'capaian_pembelajaran') || ambil(obj, 'capaian'),
      tujuan: ambil(obj, 'tujuan_pembelajaran') || ambil(obj, 'tujuan'),
      materi: ambil(obj, 'materi_elemen') || ambil(obj, 'materi'),
      kompetensi: ambil(obj, 'kompetensi'),
      indikator: ambil(obj, 'indikator_soal') || ambil(obj, 'indikator'),
      level: ambil(obj, 'level_kognitif') || ambil(obj, 'level')
    };
    /* Keterangan template yang belum ditimpa guru = dianggap kosong. */
    Object.keys(nilai).forEach(function(k) {
      if (adalahPlaceholderKartu_(nilai[k])) nilai[k] = '';
    });
    nilai.level = normalisasiLevelKognitifTeks_(nilai.level);
    var adaPerubahan = nilai.nomor || nilai.capaian || nilai.tujuan || nilai.materi ||
      nilai.kompetensi || nilai.indikator || nilai.level;
    if (!adaPerubahan) { hasil.dilewati.push('Baris ' + (i + 1) + ' (#' + id + '): tidak ada isi kartu baru — dilewati.'); continue; }
    if ((nilai.capaian || nilai.tujuan || nilai.materi || nilai.kompetensi || nilai.indikator) &&
        !soal.ks_materi && !nilai.materi) {
      hasil.dilewati.push('Baris ' + (i + 1) + ' (#' + id + '): materi/elemen wajib diisi bila mengisi kolom deskripsi — dilewati.');
      continue;
    }

    var tipe = String(soal.tipe || 'PG').toUpperCase();
    var opsiAsal = Array.isArray(soal.opsi) ? soal.opsi : null;
    if (!opsiAsal && typeof soal.opsi === 'string') {
      var urai = tryJson(soal.opsi, null);
      if (Array.isArray(urai)) opsiAsal = urai;
    }
    var normalKunci = normalisasiKunciSoal_(tipe, soal.kunci_jawaban, opsiAsal, soal.pertanyaan);
    var muatan = {
      id_soal: String(id),
      pertanyaan: String(soal.pertanyaan || ''),
      tipe: tipe,
      ks_capaian: nilai.capaian || String(soal.ks_capaian || ''),
      ks_tujuan: nilai.tujuan || String(soal.ks_tujuan || ''),
      ks_kelas: String(soal.ks_kelas || ''),
      ks_nomor_soal: nilai.nomor || String(soal.ks_nomor_soal || ''),
      ks_materi: nilai.materi || String(soal.ks_materi || ''),
      ks_kompetensi: nilai.kompetensi || String(soal.ks_kompetensi || ''),
      ks_indikator: nilai.indikator || String(soal.ks_indikator || ''),
      ks_level_kognitif: nilai.level || String(soal.ks_level_kognitif || '')
    };
    if (!muatan.ks_nomor_soal) muatan.ks_nomor_soal = nomorOtomatisKartuSoal_(id);
    if (Array.isArray(opsiAsal)) muatan.opsi = opsiAsal;
    ['poin', 'kunci_jawaban', 'mapel', 'tingkat', 'aktif',
      'stimulus_deskripsi', 'stimulus_gambar', 'stimulus_alt',
      'stimulus_video', 'stimulus_video_alt'].forEach(function(kolom) {
      if (soal[kolom] !== undefined && soal[kolom] !== null) muatan[kolom] = soal[kolom];
    });
    if (normalKunci.error) {
      hasil.dilewati.push('Baris ' + (i + 1) + ' (#' + id + '): ' + normalKunci.error);
      continue;
    }
    if (normalKunci.kunci !== '' || Object.prototype.hasOwnProperty.call(soal, 'kunci_jawaban')) {
      muatan.kunci_jawaban = normalKunci.kunci;
    }
    hasil.entri.push({ baris: i + 1, id: id, muatan: muatan });
  }
  return hasil;
}

/** Satu baris template: identitas asli soal + KETERANGAN pada semua kolom kartu
 * (guru menimpa keterangan itu dengan isi kartu soal yang sebenarnya). */
function barisTemplateKartu_(soal, peta) {
  return [soal.id_soal, peta[String(soal.id_soal)] || '', String(soal.tipe || '').toUpperCase(),
    soal.mapel || '', tingkatDariNilai_(soal.tingkat),
    truncate(SRich.stripHtml(soal.pertanyaan), 150),
    KARTU_PLACEHOLDER_.nomor_kartu, KARTU_PLACEHOLDER_.capaian_pembelajaran,
    KARTU_PLACEHOLDER_.tujuan_pembelajaran, KARTU_PLACEHOLDER_.materi_elemen,
    KARTU_PLACEHOLDER_.kompetensi, KARTU_PLACEHOLDER_.indikator_soal,
    KARTU_PLACEHOLDER_.level_kognitif];
}

/** Membangun berkas template Excel kartu soal (identitas butir + keterangan kolom kartu). */

async function buatTemplateExcelKartu_() {
  var ExcelJS = window.ExcelJS;
  if (!ExcelJS || !ExcelJS.Workbook) return null;
  var workbook = new ExcelJS.Workbook();
  workbook.creator = 'SIADO — Kartu Soal';
  workbook.properties.title = 'Template Import Kartu Soal SIADO';

  var sheet = workbook.addWorksheet('KARTU SOAL', { views: [{ state: 'frozen', ySplit: 1 }] });
  sheet.columns = KARTU_TEMPLATE_HEADERS_.map(function(h) {
    return { header: h, key: h, width: ({ id_soal: 9, nomor_bank: 11, tipe: 11, mapel: 15, kelas: 8,
      pertanyaan_ringkas: 46, nomor_kartu: 12, capaian_pembelajaran: 44, tujuan_pembelajaran: 44,
      materi_elemen: 26, kompetensi: 38, indikator_soal: 44, level_kognitif: 14 })[h] || 18 };
  });
  sheet.getRow(1).eachCell(function(cell) {
    cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF1F4E78' } };
    cell.font = { bold: true, color: { argb: 'FFFFFFFF' }, size: 11 };
  ...
  var daftar = sumberKartuGabungan_();
  var peta = petaNomorSoal_(daftar);
  var urut = urutkanSoalMenurutNomor_(daftar, peta);
  urut.forEach(function(soal) {
    var baris = sheet.addRow(barisTemplateKartu_(soal, peta));
    /* Kolom kartu (7..13) berisi keterangan — tampil miring abu-abu sebagai tanda "belum diisi". */
    for (var ci = 7; ci <= 13; ci++) {
      baris.getCell(ci).font = { italic: true, color: { argb: 'FF8A8A8A' } };
    }
  });

  var petunjuk = workbook.addWorksheet('PETUNJUK');
  petunjuk.columns = [{ width: 110 }];
  [
    'TEMPLATE IMPORT KARTU SOAL — SIADO',
    '',
    '1. Sheet "KARTU SOAL" berisi identitas butir soal Anda: kolom id_soal sampai pertanyaan_ringkas (berlatar abu-abu) JANGAN diubah atau dihapus.',
    '2. Kolom nomor_kartu sampai level_kognitif hanya berisi KETERANGAN berwarna abu-abu miring dalam kurung. GANTI keterangan itu dengan isi kartu soal yang sebenarnya.',
    '3. Baris yang keterangannya MASIH TERSISA (belum ditimpa apa pun) dianggap kosong dan DILEWATI saat import — tidak ada kartu lama yang tertimpa.',
    '4. Menghapus keterangan tanpa mengisi apa pun berarti kolom itu mempertahankan nilai kartu lama; nomor kartu kosong/terhapus = otomatis urut per mapel+kelas.',
    '5. level_kognitif boleh ditulis C1..C6 (mis. "C4 - Menganalisis") atau Taksonomi SOLO: Prastruktural / Unistruktural / Multistruktural / Relasional / Abstrak Diperluas.',
    '6. materi_elemen wajib diisi bila mengisi kolom deskripsi. Jangan menambah/menghapus baris identitas, mengganti header, atau mengubah nama sheet "KARTU SOAL".',
    '7. Simpan tetap sebagai .xlsx, kembali ke panel → Kartu Soal → Pilih Berkas → Import Kartu.',
    '',
    'Catatan: hanya baris yang keterangannya Anda timpa yang disimpan. Ringkasan import melaporkan jumlah tersimpan, dilewati, dan gagal.'
  ].forEach(function(t) { petunjuk.addRow([t]); });
...
/** Unduh template kartu soal (Excel; cadangan CSV bila ExcelJS tidak ada). */
async function unduhTemplateKartuSoal() {
  if (ADMIN.operationBusy.importKartu) return;
  var daftar = sumberKartuGabungan_();
  try {
    var blob = await buatTemplateExcelKartu_();
    if (!blob) throw new Error('Pustaka ExcelJS belum termuat.');
    unduhBlob_(blob, 'template-import-kartu-soal.xlsx');
    await hasilSukses_('Template Kartu Soal Diunduh (Excel)',
      'Template <b>template-import-kartu-soal.xlsx</b> berisi <b>' + daftar.length + '</b> butir soal Anda. ' +
      'Kolom kartu isinya baru <b>keterangan</b> (abu-abu miring) — timpa dengan isi kartu yang sebenarnya ' +
      '(lihat sheet PETUNJUK), simpan, lalu Import Kartu. Baris yang keterangannya tidak ditimpa otomatis dilewati.',
      [{ label: 'Nama berkas', nilai: 'template-import-kartu-soal.xlsx' },
       { label: 'Butir tercakup', nilai: String(daftar.length) }]);
  } catch (error) {
    var petaCsv = petaNomorSoal_(daftar);
    var rows = [KARTU_TEMPLATE_HEADERS_].concat(
      urutkanSoalMenurutNomor_(daftar, petaCsv).map(function(s) { return barisTemplateKartu_(s, petaCsv); }));
    unduhCsv_(rows, 'template-import-kartu-soal.csv');
    await hasilInfo_('Template Diunduh (CSV cadangan)',
      'Pustaka Excel belum termuat sehingga template diunduh sebagai CSV. Isi seperti biasa, lalu Import Kartu (CSV juga didukung).',
      [{ label: 'Nama berkas', nilai: 'template-import-kartu-soal.csv' }]);
  }
}

/** Import kartu soal dari berkas template. */
async function importKartuSoalDariFile() {
  var input = document.getElementById('importKartuFile');
  var kotak = document.getElementById('importKartuResult');
  var file = input && input.files ? input.files[0] : null;
  function pesan(html, kelas) {
    if (!kotak) return;
    kotak.style.display = 'block';
    kotak.innerHTML = html;
    kotak.style.borderLeftColor = kelas === 'err' ? '#c62828' : (kelas === 'ok' ? '#15803d' : '#5da3f5');
  }
  if (!file) {
    await hasilInfo_('Berkas Belum Dipilih', 'Tekan <b>Pilih Berkas</b> lalu pilih template kartu soal (.xlsx/.csv) yang sudah diisi.');
    return;
  }
  if (ADMIN.operationBusy.importKartu) return;
  ADMIN.operationBusy.importKartu = true;
  ...
  try {
    ...
    if (!rencana.entri.length) {
      throw new Error(rencana.dilewati.length
        ? 'Tidak ada baris yang dapat diproses. ' + rencana.dilewati.slice(0, 3).join(' ')
        : 'Berkas tidak berisi baris data.');
    }
    var sukses = 0, gagal = [];
    for (var i = 0; i < rencana.entri.length; i++) {
      var entri = rencana.entri[i];
      if (i % 5 === 0) {
        pesan('<i class="fa-solid fa-circle-notch fa-spin"></i> Menyimpan kartu... ' + (i + 1) + '/' + rencana.entri.length, 'info');
      }
      try {
        var hasil = await apiWajib_('simpanKartuSoal', entri.muatan);
        if (hasil && hasil.success === false) throw new Error(hasil.message || 'Ditolak server.');
        sukses++;
      } catch (galat) {
        if (isAdminSessionInvalidMessage_(galat.message || '')) throw galat;
        gagal.push('Baris ' + entri.baris + ' (#' + entri.id + '): ' + (galat.message || 'gagal.'));
      }
    }
    var html = '<i class="fa-solid fa-circle-check"></i> Import kartu selesai: <b>' + sukses + '</b> tersimpan' +
      (gagal.length ? ', <b>' + gagal.length + '</b> gagal.' : '.') +
      (rencana.dilewati.length ? '<br>Dilewati: ' + rencana.dilewati.length + ' baris.' : '') +
      (gagal.length ? '<br>' + gagal.slice(0, 10).map(escapeAdmin).join('<br>') : '') +
      (rencana.dilewati.length ? '<br><span style="font-size:11px">' + rencana.dilewati.slice(0, 10).map(escapeAdmin).join('<br>') + '</span>' : '');
    pesan(html, gagal.length ? 'info' : 'ok');
    input.value = '';
    var tombolImport = document.getElementById('importKartuSoal');
    if (tombolImport) tombolImport.disabled = true;
    await segarkanSenyap_([loadKartuSoal, loadQuestions]);
    await (gagal.length ? hasilInfo_ : hasilSukses_)(
      gagal.length ? 'Import Kartu Selesai Sebagian' : 'Import Kartu Soal Berhasil',
      (gagal.length ? 'Sebagian baris gagal disimpan — rinciannya ada di kotak hasil di panel. ' : 'Semua kartu pada berkas tersimpan. ') +
      'Gunakan filter Kelengkapan untuk memeriksa butir yang masih belum lengkap.',
      [{ label: 'Berkas', nilai: file.name },
       { label: 'Baris diproses', nilai: String(rencana.entri.length) },
       { label: 'Berhasil', nilai: String(sukses) },
       { label: 'Gagal', nilai: String(gagal.length) },
       { label: 'Dilewati', nilai: String(rencana.dilewati.length) }]);
  } catch (error) {
    pesan('<i class="fa-solid fa-circle-exclamation"></i> ' + escapeAdmin(error.message || 'Import kartu gagal.'), 'err');
...
function siapkanImportKartuUi_() {
  var input = document.getElementById('importKartuFile');
  var tombolPilih = document.getElementById('pilihFileKartu');
  var tombolImport = document.getElementById('importKartuSoal');
  var kotak = document.getElementById('importKartuResult');
  if (!input || !tombolPilih || !tombolImport) return;
  ...
  input.addEventListener('change', function() {
    ...
    if (file && kotak) {
      kotak.style.display = 'block';
      kotak.innerHTML = 'Berkas siap: <b>' + escapeAdmin(file.name) + '</b> — tekan <b>Import Kartu</b> untuk memproses.';
      kotak.style.borderLeftColor = '#5da3f5';
    }
  });
}
